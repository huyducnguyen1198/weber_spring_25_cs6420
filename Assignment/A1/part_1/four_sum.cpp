...
// while the optimized approach approximately O(n^2) at 50-500 and O(n^3logn) at 100-1000.
//
// # Detailed Analysis
...
void run_tests_parallel(int start, int end, int step) {
    ...
    for (int size = start; size <= end; size += step) {
        vector<int> arr = create_list_of_number_arithmetic(size); // Generate input data
