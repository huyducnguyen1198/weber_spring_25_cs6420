...
DepthFirstSearch::DepthFirstSearch(BaseGraph& g, std::list<int> &sources): g(g), v_attributes(new VertexAttribute[g.V()]) {
  for(int s : sources) {
    if (v_attributes[s].color == Color::White) {
		// performs a DFS from the source vertex
      	dfs(s);
    }
  }
}
...
//added the following two functions defined in .h file
void DepthFirstSearch::set_edge_type(int u, int v, EdgeType type){
	// tuple of 3 -> O(3) = O(1)
	edge_types.emplace_back(std::tuple<int, int, EdgeType>(u, v, type)); // emplace has O(1) complexity
}
void DepthFirstSearch::print_edge_types(){

	for (auto const& x : edge_types){
...
void DepthFirstSearch::dfs(int u) {
  ...
  for(int v : g.adj(u)){
    ...
    if(v_attributes[v].color == Color::White){
	  ...
	  // set edge type to tree
	  set_edge_type(u, v, EdgeType::Tree); // O(1)

	  // performs a DFS from v
      dfs(v);

    }else if(v_attributes[v].color == Color::Grey){
	  // if the vertex v is discovered but not done, sets the edge type to back
	  set_edge_type(u, v, EdgeType::Back); // O(1)
	}else{

		if (v_attributes[v].component == c_count){
			// At this point, u(the from vertices)'s component is not set yet. 
			// v(the to vertices)'s component is either not touched, or already set to some value.
			// if the vertex v is done and in the same component as u, sets the edge type to Forward
			set_edge_type(u, v, EdgeType::Forward); // O(1)
		}else{
			// if the vertex v is done and in a different component as u, sets the edge type to cross
			set_edge_type(u, v, EdgeType::Cross); // O(1)
		}
	}
  }
  ...
  if(g.is_directed()) post.push_back(u); // linked list push_back is O(1)
